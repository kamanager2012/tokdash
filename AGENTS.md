...
1. **G-1**：45 项测试 — `python3 -m unittest discover -s tests -v` 退出码 0。
...
## 5. 专业分工与集成交付（专业领域 × 工作阶段）

> 核心准则：**按专业分工审计，统一形成修复方案；按责任边界实施，按完整用户链路验收。**

1. **二维正交矩阵**：
   - **专业领域**：整体架构（模块职责/数据流/跨层根因）、前端（交互视觉/状态管理/真实浏览器行为）、后端（业务规则/真实数据持久化/重试恢复）。
   - **工作阶段**：审计（定位与客观证据）、方案（统一汇总与根因合并）、修复（边界实施与单写责任）、复验（完整用户链路验收）。
2. **动静分离证据链**：前端区分源码推断与实际浏览器 DOM/控制台网络证据；后端区分 200 OK 结构与真实落盘持久化数据。
3. **文件写入唯一责任 (Single-Writer Rule)**：跨层可以相互阅读代码，但同一文件在同一时段有且仅有唯一的写入方；共享契约与公共类型明确归属，严禁并发踩踏。
4. **主控全局收口**：严禁各专业角色各自单方面宣布“项目生产就绪”；由主控统一核验受影响的完整用户交互链路后统一出具交付结论。
