...
def build_daily_costs(period="all", refresh=True, _cache=None):
    ...
    for p, entry in cache.get("cursor", {}).items():
        if not isinstance(entry, dict):
            continue
        entry_days = entry.get("days")
        if isinstance(entry_days, dict) and entry_days:
            for dk, day_data in entry_days.items():
                if cutoff and dk < cutoff:
                    continue
                d = days.setdefault(dk, _empty())
                d["cursor"] = d.get("cursor", 0.0) + day_data.get("cost", 0.0)
                _add_day_tokens(d, dk, "cursor", day_data.get("in", 0) + day_data.get("out", 0) + day_data.get("cr", 0))
                d["sessions"] += 1
                name = f"{nice_model(day_data.get('model', 'Composer 2.5'))} (Cursor)"
                m = models.setdefault(name, {"cost": 0.0, "in": 0, "out": 0, "cr": 0, "cw": 0, "reason": 0, "tool": "cursor"})
                m["cost"] += day_data.get("cost", 0.0)
                m["in"] += day_data.get("in", 0)
                m["out"] += day_data.get("out", 0)
                m["cr"] += day_data.get("cr", 0)
        else:
            dk = entry.get("date")
            if not dk or (cutoff and dk < cutoff):
                continue
            d = days.setdefault(dk, _empty())
            d["cursor"] = d.get("cursor", 0.0) + entry.get("cost", 0.0)
            _add_day_tokens(d, dk, "cursor", entry.get("in", 0) + entry.get("out", 0) + entry.get("cr", 0))
            d["sessions"] += 1
            name = "Composer 2.5 (Cursor)"
            m = models.setdefault(name, {"cost": 0.0, "in": 0, "out": 0, "cr": 0, "cw": 0, "reason": 0, "tool": "cursor"})
            m["cost"] += entry.get("cost", 0.0)
            m["in"] += entry.get("in", 0)
            m["out"] += entry.get("out", 0)
            m["cr"] += entry.get("cr", 0)
            m["cw"] += entry.get("cw", 0)
    ...
    for _, entry in cache.get("kimicode", {}).items():
        ...
        for dk, day in entry.get("days", {}).items():
            ...
            for mn, mv in day.get("models", {}).items():
                ...
                p = _raw_price(_exact_pricing_id(_model_identity_id(mn)))
                m_cost = mv.get("cost", 0.0)
                if not m_cost and p:
                    m_cost = (mv.get("in", 0) / 1e6 * p.get("in", 0.0) +
                              mv.get("out", 0) / 1e6 * p.get("out", 0.0) +
                              mv.get("cr", 0) / 1e6 * p.get("cache_read", 0.0))
                model["cost"] += m_cost
                day_cost += m_cost
                for key in TOKEN_FIELDS:
                    model[key] += mv.get(key, 0)
